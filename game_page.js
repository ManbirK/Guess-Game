...
function word(){
    ...
    text_word = get_word.toLowerCase();
    //e.g.  titan - length is 5
    // first letter - t
    char_1 = text_word.charAt(1);

    // middle one - i
    half_length = Math.floor(word.length / 2); // 2.5 -- 2
    char_mid = text_word.charAt(half_length);

    //second last position - a
    
    char_last = text_word.charAt(word.length - 1); // 5-1 = 4

    remove_char_1 = text_word.replace(char_1, "_");
